"""
PHQ-8 AutoCoT Detailed Implementation (§3.2.2)
==============================================

Purpose: Comprehensive AutoCoT implementation for detailed PHQ-8 prediction with individual
         question-level scoring and reasoning. This is the most thorough AutoCoT approach,
         predicting all 8 PHQ-8 components plus total score.

Paper Section: §3.2.2 AutoCoT Few-Shot Prompting  
Performance: MAE 3.237, RMSE 4.257 (PHQ-8 Total) - Best AutoCoT performance
Individual Question Performance:
- PHQ_8NoInterest: MAE 0.527, RMSE 0.867
- PHQ_8Depressed: MAE 0.669, RMSE 0.976
- PHQ_8Sleep: MAE 0.574, RMSE 0.874
- PHQ_8Tired: MAE 0.574, RMSE 0.853
- PHQ_8Appetite: MAE 0.751, RMSE 1.128
- PHQ_8Failure: MAE 0.822, RMSE 1.107
- PHQ_8Concentrating: MAE 0.627, RMSE 0.973
- PHQ_8Moving: MAE 0.444, RMSE 0.781

This script implements the most comprehensive AutoCoT pipeline for PHQ-8:
1. Data Merging: Combine full transcripts with detailed PHQ-8 labels (all 8 questions)
2. Demo/Training Split: Separate demonstration examples from evaluation data
3. Detailed Rationale Generation: Generate reasoning for each PHQ-8 component
4. Multi-Score Formatting: Format demonstrations showing all 8 question scores
5. Comprehensive Inference: Predict all PHQ-8 components with step-by-step reasoning
6. Detailed Evaluation: Calculate MAE/RMSE for each individual question + total

Key Features:
- Full PHQ-8 breakdown (8 individual questions + total score)
- Comprehensive transcript processing (full concatenated text vs line-by-line)
- Detailed rationale generation for each depression symptom
- Multi-dimensional evaluation with question-level performance metrics
- Robust error handling and incremental processing

AutoCoT Strategy:
- Demonstrates reasoning for each PHQ-8 component individually
- Shows clinical reasoning: transcript evidence → symptom assessment → score
- Uses detailed prompting format requesting all 9 scores explicitly
- Comprehensive evaluation across all depression symptom dimensions

Input: E-DAIC clinical interview transcripts (full concatenated text)
Output: Complete PHQ-8 profile (8 individual scores + total, 0-3 scale per question)

Clinical Significance:
- Provides detailed depression symptom breakdown for clinical interpretation
- Enables analysis of which symptoms are easier/harder to predict
- Shows model's reasoning process for each depression component
- Most clinically useful output format for mental health assessment

Comparison Context:
- Most comprehensive AutoCoT implementation (vs baseline total-only)
- Best AutoCoT performance: MAE 3.237 vs 3.761 baseline
- Enables fine-grained analysis of depression symptom prediction
- Foundation for clinical decision support applications

Related Files:
- va_classifier.phq8.autocot.baseline — Simplified total-score-only baseline
- va_classifier.phq8.autocot.with_va — VA-enhanced version
- va_classifier.phq8.sft — Supervised fine-tuning approaches
- va_classifier.data_prep.detailed_phq8_labels.csv — Individual question labels

Usage:
    python detailed.py
    # Runs complete detailed AutoCoT pipeline with individual question scoring
    
Results:
    - Predictions: outputs/autocot_predictions.csv (all 9 PHQ-8 scores)
    - Statistics: outputs/statistics_results.txt (MAE/RMSE per question)
"""
