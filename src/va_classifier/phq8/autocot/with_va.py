"""
PHQ-8 AutoCoT with Valence-Arousal Enhancement (§3.2.2)
=======================================================

Purpose: AutoCoT few-shot prompting for PHQ-8 depression prediction enhanced with 
         valence-arousal context. This implementation demonstrates how emotional 
         context improves clinical reasoning and prediction accuracy.

Paper Section: §3.2.2 AutoCoT Few-Shot Prompting (VA-enhanced variant)
Key Innovation: Integration of continuous valence-arousal scores with transcript text

This script implements the VA-enhanced AutoCoT pipeline for PHQ-8 prediction:
1. Demo Creation: Select 8 participants with transcripts and VA scores
2. VA-Enhanced Rationale Generation: Generate reasoning using both text and emotional context
3. Multi-Modal Demo Formatting: Format demonstrations showing text + VA → reasoning → score
4. Test Processing: Prepare evaluation set with aligned transcript and VA data
5. VA-Augmented Inference: Apply AutoCoT with emotional context for each line
6. Performance Evaluation: Calculate MAE/RMSE vs baseline approaches

Key Features:
- Line-by-line VA integration (valence: -1 to 1, arousal: -1 to 1)
- Enhanced clinical reasoning incorporating emotional patterns
- Systematic comparison against text-only baseline
- Robust error handling with incremental saving
- Comprehensive evaluation pipeline

VA-Enhanced AutoCoT Strategy:
- Each demonstration shows: transcript + VA scores → clinical reasoning → PHQ-8 score
- Prompts model to analyze both content and emotional patterns
- Reasoning incorporates: "transcript content AND emotional patterns (valence/arousal)"
- Tests hypothesis: emotional context improves depression prediction

Clinical Innovation:
- Bridges gap between text analysis and affective computing
- Incorporates continuous emotional signals into clinical assessment
- Demonstrates multimodal approach to mental health prediction
- Provides interpretable reasoning chains with emotional context

Input Format:
- Line: "I feel terrible today"
- Valence: -0.8, Arousal: 0.3
- Combined context enables richer clinical reasoning

Output: PHQ-8 total scores (0-24 scale) with VA-informed reasoning

Research Significance:
- Tests core hypothesis: VA context improves clinical prediction
- Provides foundation for multimodal mental health AI
- Demonstrates practical integration of affective signals
- Enables analysis of emotional pattern importance

Comparison Context:
- Enhanced version of baseline AutoCoT (text-only)
- Compare against: Zero-shot, Detailed AutoCoT, Supervised Fine-Tuning
- Key research question: Does emotional context improve accuracy?

Related Files:
- va_classifier.phq8.autocot.baseline — Text-only baseline for comparison
- va_classifier.phq8.autocot.detailed — Comprehensive question-level scoring
- va_classifier.va_regressor.roberta_direct — VA score generation model
- va_classifier.data_prep.va_annotation — VA score preprocessing

Usage:
    python with_va.py
    # Runs complete VA-enhanced AutoCoT pipeline
    
Expected Results:
    - Improved MAE/RMSE vs text-only baseline
    - Richer clinical reasoning incorporating emotional patterns
    - Demonstration of multimodal approach effectiveness

Performance Hypothesis:
    VA-enhanced reasoning should outperform text-only baseline by incorporating
    emotional context that correlates with depression severity.
"""
...
def generate_rationale_demos_with_gpt(demo_df, output_json_path, num_demos=8, model="gpt-4o"):
    """
