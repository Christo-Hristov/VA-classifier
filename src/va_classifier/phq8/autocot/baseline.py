"""
PHQ-8 AutoCoT Baseline Implementation (§3.2.2)
==============================================

Purpose: Automatic Chain-of-Thought (AutoCoT) few-shot prompting for PHQ-8 depression prediction
         using clinical interview transcripts without valence-arousal context.

Paper Section: §3.2.2 AutoCoT Few-Shot Prompting
Performance: MAE 3.761, RMSE 5.114 (baseline without VA enhancement)

This script implements the complete AutoCoT pipeline for PHQ-8 prediction:
1. Demo Set Creation: Select 8 representative participants from training data
2. Rationale Generation: Use GPT-4 zero-shot CoT to generate reasoning examples
3. Demo Formatting: Format rationales into Q&A AutoCoT demonstration format
4. Test Set Processing: Remove demo participants, prepare remaining test cases
5. AutoCoT Inference: Apply few-shot prompting with generated demonstrations
6. Evaluation: Calculate MAE/RMSE against ground truth PHQ-8 scores

Key Features:
- Automatic demonstration selection from training participants
- Zero-shot chain-of-thought rationale generation for demos
- Line-by-line transcript processing for clinical interview analysis
- Incremental prediction saving with error recovery
- Robust evaluation metrics calculation

AutoCoT Strategy:
- Uses 8 automatically selected demonstrations with reasoning chains
- Each demo shows: transcript → step-by-step reasoning → PHQ-8 score
- Prompts GPT to "think step by step" following demonstration patterns
- No manual rationale engineering - fully automated pipeline

Input: E-DAIC clinical interview transcripts (line-by-line participant responses)
Output: PHQ-8 total scores (0-24 scale) with reasoning explanations

Comparison Context:
- Baseline for AutoCoT approach (no VA context)
- Compare against: Zero-shot, AutoCoT+VA, Supervised Fine-Tuning
- Tests effectiveness of automatic demonstration generation

Related Files:
- va_classifier.phq8.autocot.with_va — AutoCoT enhanced with VA context
- va_classifier.phq8.zero_shot.base_gpt — Zero-shot baseline
- va_classifier.phq8.sft — Supervised fine-tuning approaches
- va_classifier.data_prep.test_split.csv — Test participant splits

Usage:
    python baseline.py
    # Runs complete AutoCoT pipeline: demo creation → inference → evaluation
    
Results:
    - Predictions: outputs/autocotBASELINE_predictions.csv
    - Statistics: outputs/statistics_results.txt (MAE: 3.761, RMSE: 5.114)
"""
